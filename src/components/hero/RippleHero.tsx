...
export default function RippleHero() {
  ...
  const pointer = useRef<PointerData>({ u: .5, v: .5, active: false });
  ...
  const updatePointer = useCallback((event: React.PointerEvent<HTMLElement>) => {
    ...
    pointer.current.u = Math.min(1, Math.max(0, u));
    pointer.current.v = Math.min(1, Math.max(0, v));
    pointer.current.active = inside;
  }, []);
  const onPointerMove = useCallback((event: React.PointerEvent<HTMLElement>) => { if (event.pointerType === "touch" && !pointer.current.active) return; updatePointer(event); }, [updatePointer]);
  const onPointerDown = useCallback((event: React.PointerEvent<HTMLElement>) => { updatePointer(event); pointer.current.active = true; if (event.pointerType === "touch") event.currentTarget.setPointerCapture(event.pointerId); }, [updatePointer]);
  const endPointer = useCallback(() => { pointer.current.active = false; }, []);
